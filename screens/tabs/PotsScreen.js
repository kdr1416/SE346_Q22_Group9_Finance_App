...
export default function PotsScreen() {
  const totalSaved = mockPots.reduce((s, p) => s + p.savedAmount, 0);
  const totalTarget = mockPots.reduce((s, p) => s + p.targetAmount, 0);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            {mockPots.map(p => <PotItem key={p.id} item={p} />)}
          </View>
        </View>

        <View style={{ paddingHorizontal: Spacing.lg, marginTop: Spacing.base }}>
          <AppButton label="+ Tạo lọ tiết kiệm mới" onPress={() => {}} variant="secondary" />
        </View>
...
const styles = StyleSheet.create({
  ...
  sumLabel: { fontSize: Typography.labelMd, color: Colors.onSecondary, opacity: 0.8, marginBottom: Spacing.xs },
  sumAmount: { fontSize: Typography.displayMd, fontWeight: Typography.bold, color: Colors.onSecondary, letterSpacing: Typography.tightTracking, marginBottom: 4 },
  sumOf: { fontSize: Typography.bodyMd, color: Colors.onSecondary, opacity: 0.8 },
